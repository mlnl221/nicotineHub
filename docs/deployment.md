...
bridge scans. Mount host folders on either service (paste into `compose.override.yaml`):
...
services:
  ...
  worker:
    volumes:
      - /home/magnus/Downloads/NicotineHub:/data/NicotineHub
      - /media/500SSD:/media/500SSD:ro
# Then share /data/NicotineHub and /media/500SSD/Orpheus in Settings → Shares.
```

Any mounted path works for sharing, in-browser playback (`/api/files/raw`) and
worker tag/analyze/mediainfo — there is no allowlist. Gotcha: `/data/media/…` and
`/media/…` are different container paths — sharing the wrong one scans `0 files`
