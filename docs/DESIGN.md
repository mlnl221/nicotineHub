...
- **Language — English-only:** App is English-only by design. 30+ `po/` locales (`pynicotine/config.py:ui.language`) intentionally not ported; `UiSection.tsx` shows fixed English note. No i18n planned (see `docs/settings-mapping.md:310`, `docs/porting-status.md`).
- **Plugins — `youtube_info` not ported:** `pynicotine/plugins/youtube_info` requires YouTube Data v3 API key + `www.googleapis.com` per chat line — intentionally omitted, not homelab-relevant. `leech_detector` is ported; other plugins remain optional user plugins (`docs/plugins.md`).
- **MAX_SOCKETS — homelab-sufficient:** Fixed `512` (`env MAX_SOCKETS` configurable) intentionally not adaptive `ulimit` (`slskproto.py`); homelab sufficient.
