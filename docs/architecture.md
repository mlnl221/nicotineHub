...
1. `Login` (1): `string username`, `string password`, `uint32 major=165`, `string md5_hex(username+password)`, `uint32 minor=1` (`apps/bridge/src/soulseek.ts:187`)
2. `SetWaitPort` (2): `uint32 port` + `SharedFoldersFiles 35` (`dirs/files` from `ShareDB`) after success (nicotine parity).  
   Response `Login` (1): `bool success` → on success `string banner`, `uint32 ip`, optional `string checksum`, optional `bool isSupporter` (absent in short legacy responses — parser tolerates); on fail `string reason` ∈ {`INVALIDUSERNAME`,`EMPTYPASSWORD`,`INVALIDPASS`,`INVALIDVERSION`,`SVRFULL`,`SVRPRIVATE`} + detail.
...
userinfo {action:"watch"|"unwatch"|"get"|"interests"|"peerAddress"|...} → {type:"userinfo:event"}
...
| `DATA_DIR` | `/data` | Volume (`/data` in compose; dev falls back to `./data` or `/tmp/nicotine-hub` if `/data` not writable). Any mounted path (e.g. `/media/500SSD`) is browsable, shareable, playable and readable for metadata — no allowlist |
