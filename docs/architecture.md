...
| `ALLOWED_ROOTS` | `/data` | CSV (`:` also accepted) readable roots for `/api/files/raw` (bridge) and tag/analyze/mediainfo/audio/spectrum (worker) — must match on both services. Shares scan anywhere mounted, but playback/metadata outside these roots return `403`/`404`. E.g. `/data,/media` for a `/media/500SSD:/media/500SSD:ro` mount. Reported in `/health?json` as `allowedRoots` |
