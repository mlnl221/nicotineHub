        ...
        <div className="p-4 md:p-10 space-y-6 md:space-y-8 max-w-screen-2xl mx-auto w-full">
          ...
          <section data-testid="uploads-section" className="bg-surface dark:bg-surface-container-low rounded-xl p-4 md:p-6 ghost-border flex flex-col gap-4 max-w-full overflow-x-clip">
            ...
            <div className="flex md:hidden items-center gap-1.5">
              <button onClick={bulkAbort} disabled={!selectedTransfers.length} title="Abort selected uploads" aria-label="Abort selected uploads" className="inline-flex items-center gap-1 rounded-full bg-surface-container-high px-3 min-h-11 md:min-h-10 py-1 text-xs font-semibold disabled:opacity-40">
                <span className="material-symbols-outlined text-[16px]">block</span>
              </button>
              <button onClick={bulkRemove} disabled={!selectedTransfers.length} title="Remove selected uploads" aria-label="Remove selected uploads" className="inline-flex items-center gap-1 rounded-full bg-error-container px-3 min-h-11 md:min-h-10 py-1 text-xs font-semibold text-on-error-container disabled:opacity-40">
      ...
      {menuAnchor ? (
        <ContextMenu
          ...
          items={transferMenu({ user: menuAnchor.transfer.username, fileName: menuAnchor.transfer.fileName, virtualPath: menuAnchor.transfer.virtualPath }, true, {
            ...
            onEditTags: ["mp3","flac","ogg","m4a","wav","wma","aac","opus","aiff","aif","wv"].includes(menuAnchor.transfer.fileName.toLowerCase().split(".").pop() ?? "") ? () => setTagFile(menuAnchor.transfer.localPath || menuAnchor.transfer.fileName) : undefined,
            onAnalyzeSpectrum: ["flac","wav","aiff","aif","mp3","ogg","wma","m4a","wv","aac","opus"].includes(menuAnchor.transfer.fileName.toLowerCase().split(".").pop() ?? "") ? () => requestSpectrum(menuAnchor.transfer.id, { fileName: menuAnchor.transfer.localPath || menuAnchor.transfer.fileName }) : undefined,
            hasSpectrum: false,
          })}
