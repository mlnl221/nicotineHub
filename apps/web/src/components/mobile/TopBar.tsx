...
export function TopBar({ title = "Nicotine Hub", subtitle, showBack, backHref }: TopBarProps) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between gap-2 bg-surface-container-lowest/80 backdrop-blur-xl border-b border-outline-variant/10 px-4 pl-[calc(1rem+env(safe-area-inset-left,0px))] pr-[calc(1rem+env(safe-area-inset-right,0px))] pt-[calc(0.75rem+env(safe-area-inset-top,0px))] pb-1 shadow-sm dark:bg-surface-container-low/80 md:hidden max-w-full overflow-x-clip">
      <div className="flex items-center gap-2 min-w-0 flex-1">
        {showBack && backHref ? (
          ...
          <Link href="/search" className="flex h-8 w-8 items-center justify-center rounded-lg bg-surface-container-lowest p-0.5 shadow-sm ring-1 ring-black/5 shrink-0">
