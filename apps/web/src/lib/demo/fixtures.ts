...
export function demoReleaseScrape(url: string): import("@/lib/worker").ScrapeResult {
  ...
  return { artist: meta.artist, album: meta.album, year: meta.year, track_count: meta.track_count, query: `${meta.artist} - ${meta.album}`, source: "discogs", confidence: 0.95, url: meta.url, tracklist: [{ pos: meta.tracknumber, title: meta.title, artist: meta.artist, duration: "" }] };
}
