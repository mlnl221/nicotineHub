...
export interface TransferEvent {
  ...
  direction?: number; size?: number | bigint;
}
...
export class SoulseekSession {
    ...
    this.listener = Bun.listen({
      ...
      socket: {
        ...
        close: (peer) => {
          ...
          logger.debug("server", "peer inbound close", { username: st?.username, connType: st?.connType, remote: (peer as unknown as { remoteAddress?: string }).remoteAddress, isFileConn: (st as unknown as { isFileConn?: boolean } | undefined)?.isFileConn, fileToken: (st as unknown as { fileToken?: number } | undefined)?.fileToken });
          const fileToken = (st as unknown as { isFileConn?: boolean; fileToken?: number } | undefined);
          if (fileToken?.isFileConn && fileToken.fileToken !== undefined) {
            try { this.opts.onFileClosed?.(fileToken.fileToken); } catch {}
          }
  ...
  private processPeer(peer: Socket, chunk: ArrayBuffer | Uint8Array, initDone: boolean) {
    ...
    while (true) {
      ...
      if (msg.code === 9) {
        ...
        try { const tr = parseTransferResponse(msg.payload); this.emitTransfer({ type: "transfer-response", username: state.username, token: tr.token, reason: tr.reason, size: tr.size }); } catch {}
  ...
  transferResponse(username: string, token: number, allowed: boolean, sizeOrReason?: number | bigint | string) {
